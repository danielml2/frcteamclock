<script>
  ...
  const updateLoop = setInterval(() => {
    let currentDateString = new Date().toLocaleTimeString("en-GB", {
      hour: "numeric",
      minute: "2-digit",
    });
    if (lastDateString != currentDateString) {
      ...
      if (teamData["team_number"] != "not in TBA's API") {
        ...
        description = `
            From ${teamData["city"]}, ${teamData["country"]}${getLocation(
          teamData
        )}.
            <br /> Last competed at: ${lastSeason}, Rookie season: ${
          teamData["rookie_year"]
        }`;

        extraDetails = Object.entries(teamData).filter(
          (entry) => entry[0] != "avatarData"
        );
      } else {
  ...
  function getTeamData(teamNumber) {
    for (let index = 0; index < teamTimeData.length; index++) {
      ...
      if (team["team_number"] == teamNumber) return team;
    }
  ...
  function getLocation(teamData) {
    if (teamData["school_name"] != null) return ", " + teamData["school_name"];
  ...
  <div class="hero-content text-center">
    <div class="inset-0">
      <h3 class="text-lg py-5">
        You're watching the clock with {onlineClockWatchers - 1} other {onlineClockWatchers -
          1 >=
          2 || onlineClockWatchers - 1 == 0
          ? "people"
          : "person"}.
      </h3>
      <h2 class="text-2xl dark:text-white">
        The current time is {lastDateString} which coorelates to FRC Team:
      </h2>
      {#if avatar != "None"}
        <div class="flex flex-row justify-center">
          <img class="h-32 w-32" src={"data:image/png;base64," + avatar} />
        </div>
      {/if}
      ...
      <div class="btn-group pt-15">
        <button
          on:click={() => (moreInfoOpen = !moreInfoOpen)}
        ...
        <button
          ...
          class="btn dark:bg-blue-700 bg-blue-700 dark:text-white"
          >TBA Page</button
        >
        {#if website != undefined}
          <button
            ...
            class="btn bg-green-700 dark:bg-green-700 dark:text-white"
            >Team Website</button
          >
        {/if}
      </div>
      <div>
        <div class="dark:text-white text-md italic pt-5 pb-5">
          Powered by <a
            class="dark:text-blue-500 text-blue-500 underline"
            href="https://www.thebluealliance.com">The Blue Alliance</a
          >
        </div>
        {#if moreInfoOpen && extraDetails != undefined}
          <div class="py-6">
            <table
              class="table-fixed max-w-md bg-blue-400 dark:bg-blue-600 border-separate"
            >
              {#each extraDetails as detail}
                <tr class="dark:text-white"
                  ><td class="px-5 py-3 font-bold">{capitalize(detail[0])}</td
                  ><td class=""
                    ><div class="text-sm">
                      {detail[1] == null ? "Not specified" : detail[1]}
                    </div></td
                  ></tr
                >
              {/each}
